...
public class FileUtils {
	...
	private static String calcPath(String uploadPath) {
		...
		System.out.println("sibalwlsWK"+uploadPath+yearPath+monthPath+datePath);
	...
	private static void makeDir(String uploadPath, String... paths) {
		...
		for(String path: paths) {
			...
			// 해당 폴더가 존재하지 않으면
			if(!dirPath.exists()) {
				System.out.println("실행됫어시발");
				dirPath.mkdir(); // 폴더 생성
				
				System.out.println("폴더가 존재하는지체크true"+dirPath.exists());
			}
		}
	}
	...
	public static ResponseEntity<byte[]> getFile(String uploadPath, String fileName) throws Exception {
		...
		try {
			...
			System.out.println("이게 무슨파일일까 .." +new FileInputStream(uploadPath + fileName));
			// 파일 가져옴
